...
module vga_sync(clk, reset, h_sync, v_sync, video_on, v_count, h_count );
	...
	reg [1:0] count, d;
	reg [9:0] v_count_next, h_count_next;
	...
	always @ (*) begin
	   if(tick) begin
	       if(h_end) h_count_next = 10'b0;
	       else h_count_next = h_count + 10'b1;
	       
	       if(h_end&&v_end) v_count_next = 10'b0;
	       else if(h_end) v_count_next = v_count + 10'b1;
	   end
    end
